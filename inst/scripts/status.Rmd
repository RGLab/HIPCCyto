...
checks <- lapply(studies, function(sdy) {
  ...
  if (isTRUE(uploaded)) {
    res$uploaded <- TRUE
    res$study <- paste0("<a href='", file.path("https://fh-pi-gottardo-r-eco-public.s3-us-west-2.amazonaws.com/hipccyto", sdy, "GatingSets", latest, "study.html"), "'>", sdy, "</a>")
  } else {
    res$uploaded <- FALSE
  }
  ...
  gating_sets <- sapply(mixedsort(gs_dirs), function(gs_dir) {
    ...
    if (isTRUE(uploaded)) {
      gs_accession <- paste0("<a href='", file.path("https://fh-pi-gottardo-r-eco-public.s3-us-west-2.amazonaws.com/hipccyto", sdy, "GatingSets", latest, gs_accession, "QC.html"), "'>", gs_accession, "</a>")
    }
