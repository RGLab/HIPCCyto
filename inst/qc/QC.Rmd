      ...
      div(
        ...
        div(
          ...
          div(
            class = "sample-column",
            style = "flex: 50%; padding: 5px;",
            img(src = thisPlotInfo$spillover, style = "height: 600px; width: 100%; object-fit: contain"),
            shiny::actionButton(paste0(thisPlotInfo$name, "-correct"), "Correct lymph gate", onclick = "unflag()"),
            shiny::actionButton(paste0(thisPlotInfo$name, "-incorrect"), "Incorrect lymph gate", onclick = "flag()")
          )
        )
      )
    }
...
```{r}
tagList(
  shiny::actionButton("make-button", "Make", onclick = "create_code()"),
  pre(code(id = "impute"))
)
```
